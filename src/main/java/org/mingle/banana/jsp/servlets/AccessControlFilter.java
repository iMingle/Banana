...
/**
 * This class provides access control for all requests in the Project Billboard
 * application, by looking for the authentication token in the session and
 * forwarding to the login page if not found.
 *
...
public class AccessControlFilter implements Filter {
	...
	/**
	 * Reads the "loginPage" filter init parameter and saves the value in an
	 * instance variable.
	 *
	 * @exception ServletException
	 *                if the "loginPage" parameter is not set.
	 */
	...
	/**
	 * Looks for the authentication token in the session and forwards to the
	 * login page if not found.
	 */
	...
	private String getForwardURI(HttpServletRequest request) {
		...
		uri.append("?errorMsg=Please+log+in+first&origURL=").append(
				URLEncoder.encode(getContextRelativeURI(request)));
		return uri.toString();
	}
